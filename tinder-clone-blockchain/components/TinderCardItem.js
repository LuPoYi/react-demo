...
const TinderCardItem = ({ card }) => {
  const { handleRightSwipe, currentAccount, currentUser } = useContext(TinderContext)

  const onSwipe = (dir) => {
    if (dir === 'right') {
      handleRightSwipe(card, currentAccount, currentUser)
    }
  }

  return (
    <TinderCard className={style.tinderCardWrapper} preventSwipe={['up', 'down']} onSwipe={onSwipe}>
      <div
      ...
      >
        ...
        <div className={style.reactionsContainer}>
          ...
          <div className={`${style.xColors} ${style.buttonContainer}`}>
            <AiOutlineClose className={`${style.xColors} ${style.buttonSymbol}`} />
          </div>
